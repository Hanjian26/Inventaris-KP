     ...
     <div class="app-sidebar">
         ...
         <div class="main-sidemenu">
             ...
             <ul class="side-menu">
                 ...
                 @endif
                 @foreach ($menu as $m)
                     <?php $getMenu = AksesModel::where(['role_id' => Session::get('user')->role_id, 'menu_id' => $m->menu_id, 'akses_type' => 'view'])->count(); ?>
                     @if ($m->menu_type == 1)
                         @if ($getMenu > 0)
                             <li class="slide">
                                 <a class="side-menu__item {{ $title == $m->menu_judul ? 'active' : '' }}"
                                     data-bs-toggle="slide" href="{{ url('/admin') . $m->menu_redirect }}"><i
                                         class="side-menu__icon fe fe-{{ $m->menu_icon }}"></i><span
                                         class="side-menu__label">{{ $m->menu_judul }}</span></a>
                             </li>
                         @endif
                     @elseif($m->menu_type == 2)
                         @if ($getMenu > 0)
                             ...
                             $checkMenu = SubmenuModel::join('tbl_menu', 'tbl_menu.menu_id', '=', 'tbl_submenu.menu_id')
                                 ->select()
                                 ->where(['tbl_menu.menu_judul' => $m->menu_judul, 'tbl_submenu.submenu_judul' => $title])
                                 ->count();
                             ?>
                             <li class="slide {{ $checkMenu > 0 ? 'is-expanded' : '' }}">
                                 <a class="side-menu__item {{ $checkMenu > 0 ? 'active' : '' }}" data-bs-toggle="slide"
                                     ...
                                     <span class="side-menu__label">{{ $m->menu_judul }}</span><i
                                         class="angle fe fe-chevron-right"></i></a>
                                 <ul class="slide-menu">
                                     @foreach ($submenu as $sub)
                                         <?php $getSubmenu = AksesModel::where(['role_id' => Session::get('user')->role_id, 'submenu_id' => $sub->submenu_id, 'akses_type' => 'view'])->count(); ?>
                                         @if ($getSubmenu > 0)
                                             <li><a href="{{ url('/admin') . $sub->submenu_redirect }}"
                                                     class="slide-item {{ $title == $sub->submenu_judul ? 'active' : '' }}">{{ $sub->submenu_judul }}</a>
                                             </li>
                                         @endif
                                     @endforeach
                                 </ul>
                 ...
                 <?php $getSetting = AksesModel::where(['role_id' => Session::get('user')->role_id, 'othermenu_id' => 1, 'akses_type' => 'view'])->count(); ?>
                 <?php $getSettingMenu = AksesModel::where(['role_id' => Session::get('user')->role_id, 'othermenu_id' => 2, 'akses_type' => 'view'])->count(); ?>
                 <?php $getSettingRole = AksesModel::where(['role_id' => Session::get('user')->role_id, 'othermenu_id' => 3, 'akses_type' => 'view'])->count(); ?>
                     ...
                     <li
                         ...
                         <ul class="slide-menu">
                             ...
                             <li
                                 ...
                                 @if ($getSettingAkses == 1 || $getSettingAkses == 3 || $getSettingAkses == 4)
