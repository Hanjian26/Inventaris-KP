...
@section('content')
                    ...
                    @if ($hakTambah > 0)
                        <div>
                            <a class="modal-effect btn btn-primary-light" data-bs-effect="effect-super-scaled"
                                data-bs-toggle="modal" href="#modaldemo8">Tambah Lokasi
    ...
    <script>
        function update(data) {
            ...
            $("textarea[name='ketU']").val(data.lokasi_ket.replace(/_/g, ' '));
        }

        function hapus(data) {
            $("input[name='idlokasi']").val(data.lokasi_id);
            $("#vlokasi").html("lokasi " + "<b>" + data.lokasi_nama.replace(/_/g, ' ') + "</b>");
        }
